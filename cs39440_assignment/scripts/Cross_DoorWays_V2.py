...
def angularAdjustment(msg_twist, pub): 
	...
	if adjustment < midRangeVal/4-0.75:
		...
		clockwise = False
	if adjustment != midRangeVal/4-0.75: #midRange value by 4 is equal to 90 for the current robo configuration 
...
def crossDoorWay():
	...
	while not rospy.is_shutdown():
		...
		if(doorWayDetected == True and moveTowardsDoorway == False and not crossedDoorWay): # if doorway found set flag accordingly
			flag = 1
		elif(moveTowardsDoorway == True and crossedDoorWay == False): #move towards doorway
			flag = 2
